...
import com.weixin.tpdsl.enhancedparsing.backtrack.MismatchedTokenException;
...
public abstract class Parser {
  ...
  /**
   * Have we parsed a particular rule before at this input position? If no memoization value, we've
   * never parsed here before. If memoization value is FAILED, we parsed and failed before. If value
   * >= 0, it is an index into the token buffer.  It indicates a previous successful parse.  This
  ...
  /**
   * While backtracking, record partial parsing results. If invoking rule method failed, record that
   * fact. If it succeeded, record the token position we should skip to next time we attempt this
