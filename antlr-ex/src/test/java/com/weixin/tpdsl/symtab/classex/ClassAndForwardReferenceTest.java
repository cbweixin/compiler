...
class ClassAndForwardReferenceTest {
  ...
  @Test
  public void testLocalForwardSymbol() throws IOException, RecognitionException {
    // Create lexer/parser to build trees from stdin
    CymbolLexer lex = new CymbolLexer(new ANTLRFileStream(
        "/Users/xinwei/Documents/weixin/study-antlr/antlr-ex/src/main/java/com/weixin/tpdsl/symtab/classex/local-forward.cymbol"));
    CommonTokenStream tokens = new CommonTokenStream(lex);
    CymbolParser p = new CymbolParser(tokens);
    p.setTreeAdaptor(cymbalAdaptor);
    // launch parser by calling start rule
  ...
  @Test
  public void testInheritSymbol() throws IOException, RecognitionException {
    // Create lexer/parser to build trees from stdin
    CymbolLexer lex = new CymbolLexer(new ANTLRFileStream(
        "/Users/xinwei/Documents/weixin/study-antlr/antlr-ex/src/main/java/com/weixin/tpdsl/symtab/classex/inherit.cymbol"));
    CommonTokenStream tokens = new CommonTokenStream(lex);
    CymbolParser p = new CymbolParser(tokens);
    p.setTreeAdaptor(cymbalAdaptor);
    // launch parser by calling start rule
